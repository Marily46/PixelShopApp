  ...
  if (cart.items.length === 0) {
    return (
      <div className="container mx-auto mt-10 p-6 bg-white rounded-lg shadow-lg max-w-4xl text-center">
        ...
        <p className="text-lg mt-4"><strong>¡Empieza un carrito de compras!</strong></p>
        <button
  ...
  <div className="container mx-auto mt-10 p-6 bg-white rounded shadow-md flex flex-col lg:flex-row">
          ...
          {cart.items.map((item) => (
              <div key={item.product.id} className="py-4 flex items-center justify-between">
                  ...
                  <div className='flex-grow'>
                      ...
                      <p className="text-sm">Precio: ${item.product.price.toFixed(2)}</p>
                  </div>
      ...
      <div className='w-full lg:w-1/4 bg-gray-100 p-4 ml-0 lg:ml-6 mt-6 lg:mt-0'>
          ...
          <div className="mb-4">
              ...
              <span className="float-right">{cart.items.reduce((total, item) => total + item.quantity, 0)}</span>
          </div>
          <div className="mb-4">          
              <span>Envío:</span>
          ...
          <div className="mb-4">
              <strong>Total:</strong>
              <strong className="float-right">${cart.items.reduce((total, item) => total + item.product.price * item.quantity, 0).toFixed(2)}</strong>
          </div>
          <button
